...
import { type MachineId, PeekError, tryAsMachineId } from "./refine-externalities";
...
describe("HostCalls: Peek", () => {
  ...
  it("should fail if there is no machine", async () => {
    const { peek, registers, memory } = prepareTest(Result.error(PeekError.NoMachine));
  ...
  it("should fail if there is a page fault on any side", async () => {
    const { peek, registers, memory } = prepareTest(Result.error(PeekError.PageFault));
...
function prepareTest(result: Result<null, PeekError>) {

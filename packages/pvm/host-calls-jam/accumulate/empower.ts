...
export class Empower implements HostCallHandler {
  ...
  async execute(_gas: GasCounter, regs: Registers, memory: Memory): Promise<undefined | PvmExecution> {
    ...
    const m = tryAsServiceId(regs.get(IN_OUT_REG));
    // `a`: manages authorization queue
    const a = tryAsServiceId(regs.get(8));
    // `v`: manages validator keys
    const v = tryAsServiceId(regs.get(9));
    const sourceStart = tryAsMemoryIndex(regs.get(10));

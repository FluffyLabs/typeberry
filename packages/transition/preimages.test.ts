...
import { LookupHistoryItem, Preimages, PreimagesErrorCode, type Account, type PreimagesInput, type PreimageHash, tryAsLookupHistorySlots } from "./preimages";

function createInput(
  preimages: { requester: ServiceId; blob: BytesBlob }[],
  slot: number,
): PreimagesInput {
  return {
...
function createAccount(id: ServiceId, preimages = new HashDictionary<PreimageHash, BytesBlob>(), lookupHistory: LookupHistoryItem[] = []): Account {
  return {
    id,
    data: {
      preimages,
...
describe("Preimages", () => {
  ...
  it("should reject already integrated preimages", () => {
    ...
    const lookupHistory = [new LookupHistoryItem(hash, tryAsU32(blob.length), tryAsLookupHistorySlots([tryAsTimeSlot(5)]))];

...
import { checkAndType, Opaque } from "@typeberry/utils";
...
export function decodeStandardProgram(program: Uint8Array, args: Uint8Array) {
    ...
    const argsLength = checkAndType<number, InputLength>(args.length, args.length <= DATA_LEGNTH, 'Incorrect arguments length');
    const readOnlyLength =checkAndType<number, InputLength>(oLength, oLength <= DATA_LEGNTH, 'Incorrect readonly segment length');
    const heapLength = checkAndType<number, InputLength>(wLength, wLength <= DATA_LEGNTH, 'Incorrect heap segment length');

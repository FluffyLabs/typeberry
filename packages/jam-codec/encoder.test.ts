...
describe("JAM encoder / numbers", () => {
  ...
  it("should encode variable length u32", () => {
    ...
    assert.deepStrictEqual(
      encoder.viewResult(),
      BytesBlob.parseBlob("0x000102032af7ffffff1ff3ffffff1ff242424202")
    );
  });
  ...
  it("should encode a bunch of i32 numbers", () => {
    ...
    assert.deepStrictEqual(
      encoder.viewResult(),
      BytesBlob.parseBlob("0xffffff7f01000080424242422b00000000000000")
    );
  });
...
assert.deepStrictEqual(
      encoder.viewResult(),
      BytesBlob.parseBlob("0x4242422b0000000000")
    );
  });
...
assert.deepStrictEqual(
      encoder.viewResult(),
      BytesBlob.parseBlob("0x42422b000000")
    );
  });

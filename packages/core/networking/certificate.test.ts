...
import {ed25519AsJsonWebKeyPair, generateCertificate, verifyPeerCert} from './certificate';
...
describe('X509 Certificate', () => {
  it('should generate cert', async () => {
    ...
    assert.strictEqual(altName?.toString(), 'Subject Alternative Name:\n'
  ...
  it('should validate certificate generated by openssl', async () => {
    ...
    // when
    const verificationResult  = await verifyPeerCert([certDer], []);

    // then
    deepEqual(verificationResult, undefined);
  });
});

...
class TestStatePre067 {
  static fromJson = json.object<TestStatePre067, TestStatePre067>(
    {
      slot: "number",
      entropy: fromJson.bytes32(),
      ready_queue: [
  ...
  static toAccumulateState(
    { accounts, slot, ready_queue, accumulated, privileges }: TestStatePre067,
    chainSpec: ChainSpec,
  ): InMemoryState {
    return InMemoryState.partial(chainSpec, {
      timeslot: slot,
...
export class AccumulateTest {
  ...
  input!: Input;
  pre_state!: TestState | TestStatePre067;
  output!: Output;
  post_state!: TestState | TestStatePre067;
}
...
export async function runAccumulateTest(test: AccumulateTest, path: string) {
  ...
  const state = Compatibility.isGreaterOrEqual(GpVersion.V0_6_7)
    ? TestState.toAccumulateState(test.pre_state as TestState, chainSpec)
    : TestStatePre067.toAccumulateState(test.pre_state as TestStatePre067, chainSpec);
  ...
  const post_state = Compatibility.isGreaterOrEqual(GpVersion.V0_6_7)
    ? TestState.toAccumulateState(test.post_state as TestState, chainSpec)
    : TestStatePre067.toAccumulateState(test.post_state as TestStatePre067, chainSpec);

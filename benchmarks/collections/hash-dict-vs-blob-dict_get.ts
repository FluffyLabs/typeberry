...
export default function run() {
  ...
  for (let threshold = 0; threshold < 10; threshold++) {
    const longCollisionTestPromise = suite(
      `Comparing get operation in two hash dicts using long collition keys and BlobDictionary(${threshold})`,
    ...
    const hashKeyTestPromise = suite(
      `Comparing get operation in two hash dicts using hash keys and BlobDictionary(${threshold})`,
